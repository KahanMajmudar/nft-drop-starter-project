...
const CandyMachine = ({ walletAddress }) => {
	...
	const getProvider = () => {
		const rpcHost = process.env.REACT_APP_SOLANA_RPC_HOST
		// create connection with Solana network
		const connection = new Connection(rpcHost)
	...
	const getCandyMachineState = async () => {
		const provider = getProvider()
		// get metadata of the deployed candy machine program
		const idl = await Program.fetchIdl(candyMachineProgram, provider)
		// create a callable program instance
		const program = new Program(idl, candyMachineProgram, provider)

		// fetch metadata from the candy machine
		...
		const launchDate = candyMachine.data.goLiveDate.toNumber()
		const launchDateTimeString = `${new Date(
			launchDate * 1000
		...
		if (data.length > 0) {
			for (const mint of data) {
				...
				console.log({ 'Past Minted NFTs: ': mint })

				// avoid adding same mint data again
	...
	const renderDropDateTimer = () => {
		const currDate = new Date()
		const dropDate = new Date(machineStats.launchDate * 1000)

		if (currDate < dropDate) {
			console.log('Before Drop Time...')

			return <CountdownTimer dropDate={dropDate} />
		}

		return <p>{`Drop Date: ${machineStats.launchDateTimeString}`}</p>
	}
